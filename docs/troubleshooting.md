...
Run `baymax doctor`, then `baymax config validate`. A LAN Ollama rejection requires an explicit opt-in and firewall review. An unavailable Ollama model falls back to mock when configured. LiteRT errors name missing profile/model/tokenizer or unsupported architecture. Database errors usually indicate parent-directory permissions. For audio failure, switch to mock/console and inspect device permissions/formats.
